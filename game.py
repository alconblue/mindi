...
def create_deck(number_of_decks, number_of_players):
	...
	points = [10, 0, 0, 0, 5, 0, 0, 0, 0, 10, 10, 10, 10] if not playing_mindi else [1, 1, 1, 1, 1, 1, 1, 1, 1, 100, 1, 1, 1]
...
def doesHandHaveTwoOfClubs(hand):
	for i in hand.cards:
		if i.suit == 'clubs' and i.value == '2':
			return True
	return False
...
@login_required
def bid():
	global players, bidders, bidding_completed, game, player_order
	if playing_mindi:
		if bidding_completed:
			return redirect(url_for('app_game.play_round', round_id=1))
		hand = get_hand()
		if doesHandHaveTwoOfClubs(hand):
			...
			for i in range(1, number_of_partners+1):
				game.partners.append(Partner(suit='N/A', value='N/A', turn_number=-1, player=player_order[i*2]))
		return redirect(url_for('app_game.play_round', round_id=1))
...
def play_round(round_id):
	global player_order, player_shift, game, rounds, cards, players, past_rounds, bid_winner, player_points, playing_mindi
	...
	if len(player_order) <= player_shift:
		...
		for i, card in enumerate(table_cards):
			if card.suit==start_suit and rank[card.value] >= rank[table_cards[winner].value]:
				winner = i
			else if playing_mindi and card.suit!=start_suit and not game.trump:
...
@app_game.route('/change_game')
def change_game():
	global playing_mindi
	playing_mindi = not playing_mindi
